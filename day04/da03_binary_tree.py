...
if __name__ == '__main__':
    ...
    for name in nameAry[1:]:
        ...
        current = root
        while True:
            if name < current.data: # 현재 name이 노드안 데이터보다 작으면, 왼쪽으로 보낸다. 
                if current.left == None:
                    current.left = node
                    break # 연결했으니까 반복문 탈출
                else:
                    current = current.left # 왼쪽으로 더 내려간다.
            else: # 오른쪽으로 보낸다.
    ...
    while True:
        if delName == current.data:
            ...
            elif current.left != None and current.right == None: # 왼쪽노드에 자식이 있고, 오른쪽엔 없을 때
                if parent.left == current:
            ...
            elif current.right != None and current.left == None: # 오른쪽 노드에 자식이 있고, 왼쪽에 없을 때
