# da03_min_cost_spanningtree.py
# 최소 비용 신장 트리
from operator import itemgetter # 튜플, 리스트의 인덱스해당 아이템 가져올때 쓰는 모듈
...
while len(newAry) > (SIZE - 1): # 간선의 수가 (정점의 수 - 1) 될때까지 계속 반복
    ...
    startYN = findVertex(G, start) # 서울에 연결된 선이 있는지 확인
